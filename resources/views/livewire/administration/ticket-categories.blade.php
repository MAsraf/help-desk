<div class="w-full flex flex-col justify-start items-start gap-5">
    <div class="w-full flex md:flex-row flex-col justify-between items-start gap-2">
        <div class="flex flex-col justify-center items-start gap-1">
            <span class="lg:text-4xl md:text-2xl text-xl font-medium text-gray-700">
                @lang('Ticket categorie')
            </span>
            <span class="lg:text-lg md:text-sm text-xs font-light text-gray-500">
                @lang('Below is the list of configured tickets categories in :app', [
                    'app' => config('app.name')
                ])
            </span>
        </div>
        <button type="button" wire:click="createCategory()"
            ...
            @lang('Create a new category or subcategory')
        </button>
    </div>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            @lang($selectedCategory?->id ? 'Update category' : 'Create a new category or subcategory')
                        </h3>
                        <button wire:click="cancelCategory()" type="button"
                    ...
                    @if($selectedCategory)
                        @livewire('administration.ticket-categories-dialog', ['category' => $selectedCategory])
                    @endif
                </div>
            </div>
        </div>
    ...
    <button type="button" class="hidden" data-modal-toggle="categoryModal" id="toggleCategoryModal"></button>

    @push('scripts')
        <script>
            window.addEventListener('toggleCategoryModal', () => {
                const toggleCategoryModalBtn = document.querySelector('#toggleCategoryModal');
                toggleCategoryModalBtn.click();
            });
        </script>
    @endpush
</div>