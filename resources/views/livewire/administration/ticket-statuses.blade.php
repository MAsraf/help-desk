<div class="w-full flex flex-col justify-start items-start gap-5">
    <div class="w-full flex md:flex-row flex-col justify-between items-start gap-2">
        <div class="flex flex-col justify-center items-start gap-1">
            ...
            <span class="lg:text-lg md:text-sm text-xs font-light text-gray-500">
                @lang('Below is the list of configured tickets statuses in :app', [
                    'app' => config('app.name')
                ])
            </span>
        </div>
    ...
    <div class="w-full mt-5">
        ...
        <div id="statusModal" tabindex="-1" aria-hidden="true" data-modal-backdrop="static" wire:ignore.self
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            @lang($selectedStatus?->id ? 'Update status' : 'Create a new status')
                        </h3>
                        <button wire:click="cancelStatus()" type="button"
                    ...
                    @if($selectedStatus)
                        @livewire('administration.ticket-statuses-dialog', ['status' => $selectedStatus])
                    @endif
                </div>
            </div>
        </div>
    ...
    <button type="button" class="hidden" data-modal-toggle="statusModal" id="toggleStatusModal"></button>

    @push('scripts')
        <script>
            window.addEventListener('toggleStatusModal', () => {
                const toggleStatusModalBtn = document.querySelector('#toggleStatusModal');
                toggleStatusModalBtn.click();
            });
        </script>
    @endpush
</div>
