<div class="w-full flex flex-col justify-start items-start gap-5">
    <div class="w-full flex md:flex-row flex-col justify-between items-start gap-2">
        <div class="flex flex-col justify-center items-start gap-1">
            ...
            <span class="lg:text-lg md:text-sm text-xs font-light text-gray-500">
                @lang('Below is the list of configured user roles giving access to user of :app', [
                    'app' => config('app.name')
                ])
            </span>
        </div>
    ...
    <div class="w-full mt-5">
        <div class="w-full flex flex-col justify-start items-start gap-5">
            ...
            <div id="roleModal" tabindex="-1" aria-hidden="true" data-modal-backdrop="static" wire:ignore.self
                ...
                <div class="relative p-4 w-full max-w-4xl h-full md:h-auto">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                            ...
                            <button wire:click="cancelRole()" type="button"
                        ...
                        @if($selectedRole)
                            @livewire('administration.roles-dialog', ['role' => $selectedRole])
                        @endif
                    </div>
                </div>
            </div>
    ...
    <button type="button" class="hidden" data-modal-toggle="roleModal" id="toggleRoleModal"></button>

    @push('scripts')
        <script>
            window.addEventListener('toggleRoleModal', () => {
                const toggleRoleModalBtn = document.querySelector('#toggleRoleModal');
                toggleRoleModalBtn.click();
            });
        </script>
    @endpush
</div>
