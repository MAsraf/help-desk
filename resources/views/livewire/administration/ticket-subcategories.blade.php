<div class="w-full flex flex-col justify-start items-start gap-5">
    ...
    <div class="w-full mt-5">
        ...
        <div id="subcategoryModal" tabindex="-1" aria-hidden="true" data-modal-backdrop="static" wire:ignore.self
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            @lang($selectedSubcategory?->id ? 'Update subcategory' : 'Create a new subcategory')
                        </h3>
                        <button wire:click="cancelSubcategory()" type="button"
                    ...
                    @if($selectedSubcategory)
                        @livewire('administration.ticket-subcategories-dialog', ['subcategory' => $selectedSubcategory])
                    @endif
                </div>
            </div>
        </div>
    ...
    <button type="button" class="hidden" data-modal-toggle="subcategoryModal" id="toggleSubcategoryModal"></button>

    @push('scripts')
        <script>
            window.addEventListener('toggleSubcategoryModal', () => {
                const toggleSubcategoryModalBtn = document.querySelector('#toggleSubcategoryModal');
                toggleSubcategoryModalBtn.click();
            });
        </script>
    @endpush
</div>